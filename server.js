...
(function (port) {
    ...
    UploadHandler.prototype.get = function (req,res) {
	var albumName = req.url;
	//>>>>>>>>>>>>>>>>>>>>
		
				options.uploadDir=__dirname + '/public/files';
				options.uploadUrl='/files/';
				var albumLocation = options.uploadDir + albumName;
				var albumUrl = options.uploadUrl+albumName+'/';
				if(!(_existsSync(__dirname +albumName))){
				}
				
				if (!(_existsSync(albumLocation))) {
					console.log('dir doesnt exist>> ');
					fs.mkdirSync(albumLocation);
				}
				 
				options.uploadDir = albumLocation;
    ...
    UploadHandler.prototype.post = function () {
		...
		form.parse(handler.req, function(err,fields, files) {
				...
				options.uploadDir=__dirname + '/public/files';
				options.uploadUrl='/files/';
				var albumLocation = options.uploadDir +'/'+ fields.testType;
				var albumUrl = options.uploadUrl+fields.testType+'/';
				if (!(_existsSync(albumLocation))) {
					fs.mkdirSync(albumLocation);
				}
